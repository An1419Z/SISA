...
$sql = "SELECT Horario, Lunes, Martes, Miercoles, Jueves, Viernes FROM AudH";
$result = $enlace->query($sql);

// Consulta SQL para obtener las agendas ACEPTADAS o PENDIENTES (incluyendo la fecha)
$sqlAgendacion = "SELECT Horario, Estado, Fecha FROM Agendacion 
                   WHERE Salon = 'AUDIOVISUAL H' OR 'Audiovisual H' AND (Estado = 'ACEPTADO' OR Estado = 'PENDIENTE')"; 
$resultAgendacion = $enlace->query($sqlAgendacion);

// Almacenar los horarios reservados en un array asociativo (indexado por horario y columna)
$horariosReservados = [];
while ($rowAgendacion = $resultAgendacion->fetch_assoc()) {
    $horarioReservado = $rowAgendacion['Horario'];
    $estadoReserva = $rowAgendacion['Estado'];
    $fechaReserva = $rowAgendacion['Fecha'];

    $colorReserva = ($estadoReserva == 'ACEPTADO') ? 'red' : 'orange'; 

    $diaSemanaReserva = date('l', strtotime($fechaReserva)); 

    $columnaReserva = match ($diaSemanaReserva) {
        'Monday' => 'Lunes',
        'Tuesday' => 'Martes',
        'Wednesday' => 'Miercoles',
        'Thursday' => 'Jueves',
    ...
    if ($columnaReserva) {
        $horariosReservados[$horarioReservado][$columnaReserva] = $colorReserva;
    }
}
...
if ($result->num_rows > 0) {
    ...
    echo '<h1 align="center">AUDIOVISUAL H</h1>'; 
    ...
    echo "<tr style='background-color: black; font-family:georgia; height: 100px;'>
          ...
          while ($row = $result->fetch_assoc()) {
              ...
              foreach (['Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes'] as $dia) {
                  ...
                  if (isset($horariosReservados[$horario][$dia])) {
                      $colorCelda = $horariosReservados[$horario][$dia];
                  }
